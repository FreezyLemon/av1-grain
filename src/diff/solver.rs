...
impl NoiseModel {
    ...
    fn add_block_observations(
        ...
        let n = state.eqns.n;
        let block_w = BLOCK_SIZE / source.geometry().subsampling_x() as usize;
        let block_h = BLOCK_SIZE / source.geometry().subsampling_y() as usize;

        let dec = (
            source.geometry().subsampling_x() as usize >> 1,
            source.geometry().subsampling_y() as usize >> 1,
        );
    ...
    fn add_noise_std_observations(
        ...
        for by in 0..num_blocks_h {
            ...
            for bx in 0..num_blocks_w {
                ...
                let num_samples_h = ((frame_dims.1 / source.geometry().subsampling_y() as usize)
                    - by * block_h)
                    .min(block_h);
                let num_samples_w = ((frame_dims.0 / source.geometry().subsampling_x() as usize)
                    - bx * block_w)
                    .min(block_w);
                ...
                if num_samples_w * num_samples_h > BLOCK_SIZE {
                    let block_mean = get_block_mean(
                        ...
                        frame_dims,
                        x_o << (source.geometry().subsampling_x() >> 1),
                        y_o << (source.geometry().subsampling_y() >> 1),
                    );
                    let noise_var = get_noise_var(
                        source,
                        denoised,
                        (
                            frame_dims.0 >> (source.geometry().subsampling_x() >> 1),
                            frame_dims.1 >> (source.geometry().subsampling_y() >> 1),
                        ),
                        x_o,
                        y_o,
                        block_w,
